...
router.post("/generate-products-from-survey", async (req, res) => {
  ...
  if (
    ...
    console.log("Missing required survey data");
    return res.status(400).json({
      error: "Selected style and categories are required",
    });
  }
  ...
  try {
    ...
    console.log("Starting product generation with Gemini");

    // Use Gemini 1.5 Flash for product generation
    const model = genAI.getGenerativeModel({
      model: "gemini-2.5-flash",
    });

    // Prepare categories list including custom category if provided
    let categoriesList = selectedCategories.filter((cat) => cat !== "custom");
    if (customCategory && selectedCategories.includes("custom")) {
      categoriesList.push(customCategory);
    }

    // Prepare prompt for product recommendations
    const prompt = `
      Generate product recommendations based on the following user style preferences and categories.
      ...
      [
        {
          "type": "T-Shirt", 
          "query": "vintage graphic cotton t-shirt with retro design unisex"
        },
        {
          "type": "Jeans",
          "query": "distressed vintage wash denim jeans with faded details unisex"
        },
        {
          "type": "Sneakers",
          "query": "leather low-top sneakers minimalist design unisex"
        }
      ]

      Requirements:
      1. Generate products ONLY for the requested categories: ${categoriesList.join(
        ", "
      )}
    ...
    try {
      ...
      if (jsonMatch) {
        productItems = JSON.parse(jsonMatch[0]);
        console.log("JSON extracted successfully from response");
      } else {
        throw new Error("JSON format not found in response");
      }
      ...
      try {
        // Try parsing the entire response
        productItems = JSON.parse(responseText);
        console.log("Parsed complete response as JSON");
      } catch (e) {
        console.error("Failed to parse entire response:", e);
        ...
        return res.status(200).json({
          success: true,
          message: "Product generation completed but structured parsing failed",
          rawText: responseText,
        });
      }
    }

    // Validate and filter product items
    ...
    console.log(`Generated ${validItems.length} valid product recommendations`);

    // If no valid items, return an error
    if (validItems.length === 0) {
      console.log("All product items were invalid or incomplete");

      return res.status(500).json({
        success: false,
        error: "Failed to generate products properly",
        details: "Gemini API did not return any valid product recommendations",
      });
    }
